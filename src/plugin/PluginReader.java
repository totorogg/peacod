...
public class PluginReader {
	...
	public PluginReader() {
		...
		StringBuffer pluginDirPath = new StringBuffer(System
				.getProperty("user.dir")).append("/plugin");
		System.out.println(pluginDirPath);
	...
	public void search(String directory) throws Exception {
		...
		for (File f : files) {
			...
			for (PluginInfo aPlugin : plugInfoList) {
				...
				String interfaceURL = new String(aPlugin.path + "."
						+ aPlugin.className);
				interfaceURL = interfaceURL.substring(0,
						interfaceURL.length() - 6);
				...
				for (Class c : interfaces) {
					if (c.getName().equals(
							"plugin.PlugInterface")) {
						pluginInterfaceList.add((PlugInterface) clazz
								.newInstance());
					}
				}
			}
	...
	protected List<String> getClassNames(JarInputStream jarFile)
			throws IOException {
...
class ConfigXML {
	...
	public static String getTagValue(String tagName, Element aElement) {
		...
		NodeList nList = aElement.getElementsByTagName(tagName).item(0)
				.getChildNodes();
